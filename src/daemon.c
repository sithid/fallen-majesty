...
void do_leech(CHAR_DATA *ch, char *argument)
{
	...
	act("$n stares intently at $N.", ch, NULL, victim, TO_NOTVICT, FALSE);
	act("You stare intently at $N.", ch, NULL, victim, TO_CHAR, FALSE);
	act("$n stares intently at you.", ch, NULL, victim, TO_VICT, FALSE);

      if (victim->hit >= 10000)
      {
	  int dam = ch->damroll;
          if (dam < 0) dam = 1000;
	  if (dam > 5000) dam = 5000 + (dam / 10);
	  damage(ch,victim,dam,gsn_leech);
	  ch->hit += dam;
...
void do_gust( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_NPC(ch) && number_percent( )/2 > ch->power[DISC_DAEM_GELU] + ch->generation)
    {
	    ...
	    act("$n blows a gust of cold air into the room.", ch, NULL, NULL, TO_ROOM, FALSE);
	    return;
    }
    SET_TIMER(ch, TIMER_CAN_GUST, dice(1,3)+1);    
    act("You breathe a gust of cold air at $N.", ch, NULL, victim, TO_CHAR, FALSE);
    act("$n breathes a gust of cold air at $N.", ch, NULL, victim, TO_NOTVICT, FALSE);
    act("$n breathes a gust of cold air at you.", ch, NULL, victim, TO_VICT, FALSE);
...
void do_caust(CHAR_DATA *ch, char *argument)
{
    ...
    if (dice(1, 100) == 1)
    {
	act("$p is disintegrated by $n's poison.", ch,obj, NULL, TO_ROOM, FALSE);
	act("Your poison eats through $p.", ch,obj, NULL, TO_CHAR, FALSE);
	extract_obj(obj);
	return;
    }
    ...
    {
	act("$p is already coated with deadly poison.", ch, obj, NULL, TO_CHAR, FALSE);
	return;
    }

    act("You run your tongue along $p, coating it with a sickly venom.", ch, obj, NULL, TO_CHAR, FALSE);
    act("$n runs $m tongue along $p, coating it with a sickly venom.", ch, obj, NULL, TO_ROOM, FALSE);
...
void do_chaosport( CHAR_DATA *ch, char *argument )
{
    ...
    if (IS_NPC(victim))
    {
	stc("Not while fighting mobs, coward!\n\r",ch);
	return;
    }
    if (IS_SET(ch->in_room->room_flags, ROOM_ARENA))
    {
	stc("You are trying to get away? FLEE!\n\r",ch);
        return;
    }
	...
	if (number_range(1,10) <= 6)
	{
	    ...
	    act("$n tries in vain to summon a chaos portal.",ch,NULL,NULL,TO_ROOM, FALSE);
	    SET_TIMER(ch, TIMER_CHAOSPORT, 2);
	    WAIT_STATE(ch, 14);
	    return;
	}
    else {
    act("$n disappears through a rift in time.",ch,NULL,NULL,TO_ROOM, FALSE);
    ...
    act("$n appears in a swirling wind of Chaos.",ch,NULL,NULL,TO_ROOM, FALSE);
    do_look(ch,"auto");
	WAIT_STATE(ch, 18);
...
void do_wfreeze(CHAR_DATA *ch, char *argument)
{
    ...
    if (dice(1, 100) == 1)
    {
	act("$p freezes and shatters.", ch,obj, NULL, TO_ROOM, FALSE);
	act("$p freezes and shatters.", ch,obj, NULL, TO_CHAR, FALSE);
	extract_obj(obj);
	return;
    }
    ...
    {
	act("$p is already frozen.", ch, obj, NULL, TO_CHAR, FALSE);
	return;
    }

    act("$p is surrounded by ice crystals.", ch, obj, NULL, TO_CHAR, FALSE);
    act("$p, carried by $n is surrounded by ice crystals.", ch, obj, NULL, TO_ROOM, FALSE);
...
*/
    ...
    act("$n summons the Abyssal fires of Hell!",ch,NULL,NULL,TO_ROOM, FALSE);
}
...
*/
    ...
    act("$p starts to shudder and vibrate in your hands.", ch, obj, NULL, TO_CHAR, FALSE);
    act("$n lays $s hands on $p.", ch, obj, NULL, TO_ROOM, FALSE);
...
void do_immolate(CHAR_DATA *ch, char *argument)
{
  ...
  {
    send_to_char("You can only immolate items you own.\n\r",ch);
    return;
  }
    ...
    if (dice(1, 100) == 1)
    {
	act("$p explodes in a burst of flames.", ch,obj, NULL, TO_ROOM, FALSE);
	act("$p explodes in a burst of flames.", ch,obj, NULL, TO_CHAR, FALSE);
    ...
    {
	act("$p is already flaming.", ch, obj, NULL, TO_CHAR, FALSE);
	return;
    }

    act("$p bursts into flames.", ch, obj, NULL, TO_CHAR, FALSE);
    act("$p, carried by $n bursts into flames.", ch, obj, NULL, TO_ROOM, FALSE);
...
void do_dinferno(CHAR_DATA *ch, char *argument)
{
    ...
    act("$n explodes, sending forth a massive fireballs in all directions.",ch,NULL,NULL,TO_ROOM, FALSE);
...
void fire_effect(void *vo, int level, int dam, int target)
{
	...
	/* chance of blindness */
	if (!IS_AFFECTED(victim,AFF_BLIND) && !IS_NPC(victim))
	{
	    AFFECT_DATA af;
...
void do_blink(CHAR_DATA *ch, char *argument)
{
   ...
   if (ch->power[DISC_DAEM_ATTA] < 7)
	...
	act("$n pops out of existance.", ch, NULL, NULL, TO_ROOM, FALSE);
...
void do_graft(CHAR_DATA *ch, char *argument)
{
    ...
    {
	act("You graft an arm onto your body.",ch,NULL,obj,TO_CHAR, FALSE);
	act("$n grafts an arm onto $m body.",ch,NULL,obj,TO_ROOM, FALSE);
 ...
 void do_dgate( CHAR_DATA *ch, char *argument )
{
	...
	else
	{
		for ( d = first_descriptor; d != NULL; d = d->next )
		{
			...
			if ( d->character->fighting != NULL ) stop_fighting( d->character, TRUE );
			    act( "A scaly hand reaches down and drags $n away.", d->character, NULL, NULL, TO_ROOM, FALSE );
			    char_from_room( d->character );
			    char_to_room( d->character, ch->in_room );
			    act( "You have successfully gated $N.", ch, NULL, d->character, TO_CHAR, FALSE );
			    act( "$n has successfully gated $N.", ch, NULL, d->character, TO_NOTVICT, FALSE );
			    if ( ch != d->character )
				act( "$n has pulled you through the demonic void.", ch, NULL, d->character, TO_VICT , FALSE);
    ...
    act( "A scaly hand reaches down and drags $n away.", victim, NULL, NULL, TO_ROOM, FALSE );
    char_from_room( victim );
    char_to_room( victim, ch->in_room);
    act( "You have successfully gated $N.", ch, NULL, victim, TO_CHAR , FALSE);
    act( "$n has successfully gated $N.", ch, NULL, victim, TO_NOTVICT, FALSE );
    if ( ch != victim )
	act( "$n has pulled you through the demonic void.", ch, NULL, victim, TO_VICT , FALSE);
    do_look( victim, "auto" );
    send_to_char( "Ok.\n\r", ch );
}
...
void do_frostbreath (CHAR_DATA *ch, char *argument)
{
	...
	WAIT_STATE(ch, 24);
	SET_TIMER(ch, TIMER_CAN_BREATHE_FROST, 11);
    ...
    WAIT_STATE(ch, 12);
    SET_TIMER(ch, TIMER_CAN_BREATHE_FROST, 9 );
...
void frost_breath(CHAR_DATA *ch, CHAR_DATA *victim, bool all)
{
	...
	act("$n breathes forth a stream of frost over you.",ch,NULL,NULL,TO_ROOM, FALSE);
	act("You breath forth a stream of frost.",ch,NULL,NULL,TO_CHAR, FALSE);
	...
	for (vch = ch->in_room->people; vch != NULL; vch = vch_next)
	{
		...
		cold_effect(vch,0,dam,TARGET_CHAR);
    ...
    if (is_safe(ch, victim)) return;
    dam = ((10 - ch->generation +3) * power+3) * 50;

    act("$n breathes forth a stream of frost over $N.",ch,NULL,victim,TO_NOTVICT, FALSE);
    act("$n breathes a stream of frost over you!",ch,NULL,victim,TO_VICT, FALSE);
    act("You breath forth a stream of frost over $N.",ch,NULL,victim,TO_CHAR, FALSE);
