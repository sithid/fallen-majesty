...
/*   if ( d->connected != CON_EDITING ) 
   ...
   {
	send_to_char( "You can't do that!\n\r", ch );
      sprintf(buf,"Edit_buffer: illegal ch->substate (%s : %d)", ch->name, ch->substate
);
      bug(buf,0);
...
bool is_social( CHAR_DATA *ch, char *argument)
{
   int cmd;
    bool found;
    ...
    {
        if ( argument[0] == social_table[cmd].name[0]
        &&   !str_prefix( argument, social_table[cmd].name ) )
        {
            found = TRUE;
            break;
        }
...
void do_help( CHAR_DATA *ch, char *argument )
{
    ...
    {
       if ( is_social( ch, arg1 ) )
       {
	 do_sochelp(ch, arg1);
         return;
       }
	send_to_char( "There is no helpfile matching that word, the immortals have been notified.\n\r", ch );
	sprintf(log,"Log %s: help %s, does not exist.",ch->pcdata->switchname,argument);
	log_string(log);
	return;
    }
    if ( pHelp->level >= -1)
