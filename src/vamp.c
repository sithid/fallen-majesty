...
void do_vampirearmor( CHAR_DATA *ch, char *argument )
{
  ...
  if (arg[0] == '\0')
  {
    send_to_char("Please specify which piece of vampire armor you wish to make: Ring Collar Plate Helmet\n\rLeggings Boots Gloves Sleeves Cape Belt Bracer Visor Dagger Longsword.\n\r",ch);
    return;
  }

    if ( ch->practice < 150 )
    {
        send_to_char("It costs 150 points of primal to create vampire equipment.\n\r",ch);
        return;
    }
...
void do_preserve( CHAR_DATA *ch, char *argument )
{
    ...
    if ( obj->item_type == ITEM_HEAD || obj->item_type == ITEM_CORPSE_NPC)
    {
    ...
    obj->timer = -1;
    act("You place your hands on $p and concentrate on it.",ch,obj,NULL,TO_CHAR, FALSE);
    act("$n places $s hands on $p and it glows brightly.",ch,obj,NULL,TO_ROOM, FALSE);
    return;
}
...
void do_spiritgate( CHAR_DATA *ch, char *argument )
{
  ...
  if ( room_is_private(obj->in_room ) )
  {
    send_to_char( "That room is private right now.\n\r", ch );
    return;
  }
  if ( room_is_noft(obj->in_room ) && ch->fight_timer >0 )
  {
    ...
    act("$n steps into a spirit gate and vanishes.",ch,NULL,NULL,TO_ROOM, FALSE);
    char_from_room(ch);
    char_to_room(ch, get_room_index( obj->in_room->vnum ));
    act("You step through a spirit gate and appear before $p.",ch,obj,NULL,TO_CHAR, FALSE);
    act("$n steps out of a spirit gate in front of $p.",ch,obj,NULL,TO_ROOM, FALSE);
...
stc("You are too exhausted to purify your mind\n\r",ch);
    ...
    act( "A bright halo glows above $n's head.", ch, NULL,NULL, TO_ROOM, FALSE );
...
/* Removed safe room restriction for scream ability */
   ...
   {
     stc("You cannot do this at the moment.\n\r",ch);
     return;
    }

              if ( number_range( 1, (12 - ch->power[DISC_VAMP_MELP])) == 2 )
              {
                act("$n lets out an ear-popping scream!",ch,NULL,vch,TO_ROOM, FALSE);
                stc("You fall to the ground, clutching your ears.\n\r",vch);
                vch->position = POS_STUNNED;
                act("$n falls to the ground, stunned.",vch,NULL,vch,TO_NOTVICT, FALSE);
                return;
              }
	      else
	      {
...
if (ch->embracing != NULL)
{
    ...
    act("$N retracts his fangs.",ch,NULL,NULL,TO_ROOM, FALSE);
...
send_to_char(buf,ch);act(buf,ch,NULL,NULL,TO_ROOM, FALSE);
...
void do_fear( CHAR_DATA *ch, char *argument )
{
    ...
    {
	sprintf(buf,"%s is not afraid of you.",arg);
	stc(buf, ch);
	return;
    }
    if ( IS_NPC(victim) )
    {
      act("You bare your fangs and growl at $N.",ch,NULL,victim,TO_CHAR, FALSE);
      act("$n bares $s fangs and growls at you.",ch,NULL,victim,TO_VICT, FALSE);
      act("$n bares $s fangs and growls at $N, and $N flees in terror.",ch,NULL,victim,TO_NOTVICT, FALSE);
      do_flee( victim, "" );
      return;
    }
    ...
    if ( !IS_NPC(victim) )
    {
      if ( ch->power[DISC_VAMP_DAIM] < 5 )
      {
        if ( number_range( 1, 4 ) != 2 )
        {
          act("You bare your fangs and growl at $N, but nothing happens.",ch,NULL,victim,TO_CHAR, FALSE);
          act("$n bares $s fangs and growls at you.",ch,NULL,victim,TO_VICT, FALSE);
          act("$n bares $s fangs and growls at $N, but nothing happens.",ch,NULL,victim,TO_NOTVICT, FALSE);
	  return;
        }
      }
...
void do_sharpen(CHAR_DATA *ch, char *argument)
{
  ...
  act(arg,ch,NULL,NULL,TO_ROOM, FALSE);
  return;
}
...
void do_gourge(CHAR_DATA *ch, char *argument  )
{
   ...
   return;
    ...
    if (ch->pcdata->condition[COND_THIRST] >= 1000 /(ch->generation +1))
    {
    send_to_char("Your bloodlust is sated.\n\r",ch);
    ch->pcdata->condition[COND_THIRST] = 1000/(ch->generation +1);
    }
    sprintf(buf,
 ...
 if ( ( victim = get_char_room( ch, arg1 ) ) == NULL )
{
  ...
  if (!IS_NPC(victim)) dam=4000;
  if (!IS_CLASS(victim,CLASS_VAMPIRE)) hurt_person(ch,victim,dam);
    act("$N screams in agony as you turn his blood to water.",ch, NULL, victim,TO_CHAR, FALSE);
    act("$N screams in agony as $n turns his blood to water.",ch, NULL, victim,TO_NOTVICT, FALSE);
    act("You scream in agony as $n turns your blood to water.", ch, NULL, victim,TO_VICT, FALSE);
...
{
xREMOVE_BIT(ch->extra,EXTRA_DRAGON);
act( "You transform back into zuloform.", ch, NULL, NULL, TO_CHAR, FALSE );
    act( "$n shrinks back into a big black monster.", ch, NULL, NULL,
TO_ROOM, FALSE );
    
 
if (ch->hit < 1) ch->hit = 1;
    ch->damroll = ch->damroll - 800;
    ch->hitroll = ch->hitroll - 800;
...
ch->pcdata->condition[COND_THIRST] -= number_range(200,400);
    act( "You transform into a large dragon.", ch, NULL, NULL, TO_CHAR, FALSE );
    act( "$n's body grows and distorts into a large dragon.", ch, NULL,
NULL,TO_ROOM, FALSE);
xSET_BIT(ch->extra,EXTRA_DRAGON);
...
{send_to_char("You are unable to take the form of that.\n\r",ch);return;}
    ...
    act("$n's form shrinks and distorts into $p.",ch,obj,NULL,TO_ROOM, FALSE);
    act("Your form shrinks and distorts into $p.",ch,obj,NULL,TO_CHAR, FALSE);
...
void do_facade( CHAR_DATA *ch, char *argument )
{
	...
	xSET_BIT(ch->extra, EXTRA_FAKE_CON);
	...
	xREMOVE_BIT(ch->extra, EXTRA_FAKE_CON);
...
void do_zombie(CHAR_DATA *ch, char *argument )
{
   ...
   if (ch->pcdata->powers[NECROMANTIC] < 1 && IS_CLASS(ch, CLASS_POWER_LICH))
   {
    ...
    xSET_BIT(victim->extra, EXTRA_ZOMBIE);
   ...
   if (victim->level > 5000) victim->level = 5000;
 if (victim->level < 100)
{
...
if (!IS_NPC(victim))
{
dam = ch->power[DISC_VAMP_OBTE] * 200;
}
else if (IS_NPC(victim))
{
dam = ch->power[DISC_VAMP_OBTE] * 900;
}
...
act( "A clone of $p appears in your hands in a flash of light.", ch, obj, NULL, TO_CHAR, FALSE );
    act( "A clone of $p appears in $n's hands in a flash of light.", ch, obj, NULL, TO_ROOM, FALSE );
...
if (!IS_NPC(victim))
{
dam = ch->power[DISC_VAMP_SERP] * 400;
}
else if (IS_NPC(victim))
{
dam = ch->power[DISC_VAMP_SERP] * 1000;
}
if ( !IS_NPC(victim) && IS_CLASS(victim, CLASS_WEREWOLF) )
{
    if (victim->power[DISC_WERE_BOAR] > 2 ) dam *= 0.5;
...
void do_objmask( CHAR_DATA *ch, char *argument )
{
    ...
    SET_BIT(ch->affected_by, AFF_POLYMORPH);
    act("You mask yourself as $p.",ch,obj,NULL,TO_CHAR, FALSE);
    act("$n masks $mself as $p.",ch,obj,NULL,TO_ROOM, FALSE);
    free_string(ch->morph);
    ch->morph = str_dup(obj->short_descr);
...
void do_mirror( CHAR_DATA *ch, char *argument )
{
	...
	char_to_room( victim, ch->in_room );
	act( "You concentrate your powers and form a mirror image of yourself.", ch, NULL, victim, TO_CHAR, FALSE );
	act( "$n waves $s hands and a mirror image of $mself appears.", ch, NULL, victim, TO_ROOM, FALSE );
	return;
...
void do_control( CHAR_DATA *ch, char *argument )
{
    ...
    victim->wizard = ch;
    act("You take control of $N's mind.",ch,NULL,victim,TO_CHAR, FALSE);
    act("$n takes control of $N's mind.",ch,NULL,victim,TO_ROOM, FALSE);
...
void do_formillusion( CHAR_DATA *ch, char *argument )
{
	...
	act( "You concentrate your powers and form an illusion of yourself.", ch, NULL, victim, TO_CHAR, FALSE );
	act( "$n waves $s hands and splits in two.", ch, NULL, victim, TO_ROOM, FALSE );
	WAIT_STATE(ch,20);
	return;
...
void do_ashes( CHAR_DATA *ch, char *argument )
{
    ...
    act("You transform into $p and falls to the ground.",ch,obj,NULL,TO_CHAR, FALSE);
...
void do_astralwalk( CHAR_DATA *ch, char *argument)
{
 ...
 if( IS_SET(victim->in_room->room_flags, ROOM_ASTRAL))
  ...
  if( IS_SET(ch->in_room->room_flags, ROOM_ASTRAL))
  {
    ...
    act("You step through a rift in the Astral plane.\n\rYou leap out of the Astral Plane before $N.", ch, NULL, victim, TO_CHAR, FALSE);
    act("$n leaps out of a rift in the Astral Plane.", ch, NULL, victim, TO_NOTVICT, FALSE);
    act("$n leaps out of a rift in the Astral Plane.", ch, NULL, victim, TO_VICT, FALSE);
    do_look(ch,"auto");
    return;
}
...
void do_beckon(CHAR_DATA *ch, char *argument  )
{
    ...
    act( "You whistle loudly and $N walks in.", ch, NULL, victim, TO_CHAR, FALSE);
    act( "$n whistles loudly and $N walks in.", ch, NULL, victim, TO_ROOM, FALSE);
    char_to_room( victim, ch->in_room );
...
send_to_char( "You aren't fighting anyone.\n\r", ch );
    ...
    if (!IS_NPC(victim))
    {
    dam = ch->power[DISC_VAMP_QUIE] * 500;
    } 
    else if (IS_NPC(victim))
    {
    dam = ch->power[DISC_VAMP_QUIE] * 1000;
    }
...
*/ 
    ...
    act( "You close your eyes and concentrate a huge serpent walks in.", ch, NULL, victim, TO_CHAR, FALSE);
    act( "$n closes his eyes and concentrates a huge $N walks in.", ch, NULL, victim, TO_ROOM, FALSE);
    char_to_room( victim, ch->in_room );
    ch->pcdata->followers += 1;
    xSET_BIT(victim->act, ACT_NOEXP);
...
void do_illusion(CHAR_DATA *ch, char *argument  )
{
    ...
    act( "You close your eyes and concentrate an illusion appears in the room.", ch, NULL, victim, TO_CHAR, FALSE);
    act( "$n closes his eyes and concentrates an $N appears in the room.", ch, NULL, victim, TO_ROOM, FALSE);
    char_to_room( victim, ch->in_room );
    ch->pcdata->followers += 1;
    xSET_BIT(victim->act, ACT_NOEXP);
