...
void do_medit( CHAR_DATA *ch, char *argument )
{
	...
	case SUB_MOB_DESC:
	  ...
	  pMob = ch->dest_buf;
	  STRFREE( pMob->description );
	  pMob->description = copy_buffer( ch );
	  stop_editing( ch );
	  ch->substate = SUB_NONE;
	  return;
    }
  ...
  "\n\r", ch );
	...
	case 'M':
		...
		send_to_char(buf,ch);
            ...
            pMob = pMobIndex;
            cprintf( buf, "M[%5d] %-13.13s#n in room             R[%5d] [%3d] %-15.15s\n\r",
	...
	case 'O':
		...
		send_to_char(buf,ch);
            ...
            cprintf( buf, "O[%5d] %-13.13s#n in room             "
                          "R[%5d]       %-15.15s\n\r",
	...
	case 'P':
	    ...
	    cprintf( buf,
		"O[%5d] %-13.13s#n inside              O[%5d]       %-15.15s\n\r",
		pReset->arg1,
		pObj->short_descr,
		pReset->arg3,
	...
	case 'E':
	    ...
	    {
	    cprintf( buf,
		"O[%5d] %-13.13s#n in the inventory of S[%5d]       %-15.15s\n\r",
		pReset->arg1,
		pObj->short_descr,                           
		pMob->vnum,
		pMob->short_descr  );
	    ...
	    cprintf( buf,
		"O[%5d] %-13.13s#n %-19.19s#n M[%5d]       %-15.15s\n\r",
		pReset->arg1,
		pObj->short_descr,
		(pReset->command == 'G') ?
	...
	case 'D':
	    ...
	    cprintf( buf, "R[%5d] %s door of %-19.19s reset to %s\n\r",
		pReset->arg1,
