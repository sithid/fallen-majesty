...
void do_lloth( CHAR_DATA *ch, char *argument )
{
  ...
  for ( gch = char_list; gch != NULL; gch = gch->next)
  {
    ...
    if (!IS_CLASS(gch, CLASS_DROW) && !IS_CLASS(gch, CLASS_CYBORG)) continue;
...
void do_grant( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg1[0] == '\0' || arg2[0] == '\0' )
	{
		...
		send_to_char("#pShadowblast #0(#r25000#0)#n, #pConfuse #0(#r2500#0)#n\n\r",ch);
  ...
  {		send_to_char( "They already have a profession.\n\r", ch );
	...
	{inpart = DPOWER_FIGHTDANCE; cost =10000;}
        else if (!str_cmp(arg2,"shadowblast"))
        {inpart = DPOWER_SHADOWBLAST; cost =25000;}   
    else
	{
               send_to_char( "Syntax: Grant <person> <power>\n\r", ch );
                send_to_char("#pDrowfire #0(#r2500#0)#n,  #pDarkness #0(#r7500#0)#n,  #pDrowpoison #0(#r2500#0)#n.\n\r",ch);
...
void do_chaosblast(CHAR_DATA *ch, char *argument) {
    ...
    level = ch->spl[spelltype];
        act("You concentrate your power on $N.",ch,NULL,victim,TO_CHAR, FALSE);
        act("$n concentrates $s power on you.",ch,NULL,victim,TO_VICT, FALSE);
...
void do_drowcreate( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ch->practice < 150)
    {
	send_to_char("It costs 150 points of primal to create a piece of drow armour.\n\r",ch);
	return;
    }
...
void do_heal ( CHAR_DATA *ch, char *argument ) 
{
	...
	ch->mana = ch->mana - 2000;
	ch->hit = ch->hit + ch->spl[BLUE_MAGIC] * number_range(10,20);
	if (ch->hit > ch->max_hit) ch->hit = ch->max_hit;
	send_to_char("Lloth heals you.\n\r",ch);
	act("$n is healed by Lloth.",ch,NULL,NULL,TO_ROOM, FALSE);
	WAIT_STATE(ch, 20);
	return;
}
 void do_shadowblast( CHAR_DATA *ch, char *argument ) {
    ...
    int lag;
        ...
        if(!IS_CLASS(ch, CLASS_DROW) ||!IS_SET(ch->pcdata->powers[1],DPOWER_SHADOWBLAST)) {
                send_to_char("Huh?\n\r", ch );
                return;}
    ...
    return;
      ...
      if (is_safe(ch, victim)) return;
        ...
        act("$n throws a ball of swirling shadows at $N.", ch, NULL, victim, TO_NOTVICT, FALSE);
        act("You throw a ball of swirling shadows at $N.", ch, NULL, victim, TO_CHAR, FALSE);
        act("$n throws a ball of swirling shadows in your face.", ch, NULL, victim, TO_VICT, FALSE);

      if (victim->hit >= 10000)
      {
          int dam;
          dam = ch->damroll + ch->hitroll;
	  if (dam < 0) dam = 1000;
          if (dam > 10000) dam = 10000 + (dam / 10);
          damage(ch,victim,dam,gsn_shadowblast);
          ch->hit += dam;
          if (ch->hit > ch->max_hit)
                ch->hit = ch->max_hit;
          sprintf(buf,"#0Your shadows consume #R%d#0 hitpoints.#n\n\r",dam);
          stc(buf,ch);
          sprintf(buf,"#0%s's shadows consumed #R%d#0 of your hitpoints!#n\n\r",ch->name,dam);
          stc(buf,victim);
      }
      else stc("Nothing seemed to happen.\n\r",ch);
	lag = number_range(0,4);
	switch(lag)
	{
	 case 0:
	  WAIT_STATE(ch, 14);
...
/*	if (!IS_NPC(victim)) {
	...
	act("You walk into the shadows.", ch, NULL, NULL, TO_CHAR, FALSE);
	act("$n walks into the shadows.", ch, NULL, NULL, TO_ROOM, FALSE);
	ch->move -= 250;
	char_from_room(ch);
	char_to_room(ch, location);
	do_look(ch, "auto");	

	act("You walk out of the shadows.", ch, NULL, NULL, TO_CHAR, FALSE);
	act("$n walks out of the shadows.", ch, NULL, NULL, TO_ROOM, FALSE);
...
void do_spiderform( CHAR_DATA *ch, char *argument ) 
{
	...
	act("You mutate into a giant spider.",ch,NULL,NULL,TO_CHAR, FALSE);
        act("$n mutates into a giant spider.",ch,NULL,NULL,TO_ROOM, FALSE);
...
void do_darkness(CHAR_DATA *ch, char *argument) 
{
	...
	if ((!IS_CLASS(ch, CLASS_DROW) && !IS_CLASS(ch, CLASS_CYBORG))
        || (!IS_SET(ch->pcdata->powers[1], DPOWER_DARKNESS) && !IS_CLASS(ch, CLASS_CYBORG)))
        {
	...
	act("$n summons a globe of darkness.",ch,NULL,NULL,TO_ROOM, FALSE);
	ch->mana -= 1000;
...
void do_confuse(CHAR_DATA *ch, char *argument) {
	...
	if (IS_NPC(ch)) return;
	if (!IS_CLASS(ch, CLASS_DROW) && !IS_CLASS(ch, CLASS_PSIONIST))
	{
	stc("Huh?\n\r",ch);
	return;
	}
	if (IS_CLASS(ch, CLASS_DROW) &&	!IS_SET(ch->pcdata->powers[1], DPOWER_CONFUSE)) {
	send_to_char("Perhaps you should acquire that power?\\n\r",  ch);
	return;}
	if (IS_CLASS(ch, CLASS_PSIONIST) && !IS_SET(ch->pcdata->powers[PSIONIST_POWER], PSIONIST_CONFUSE))
        {
        stc("Perhaps you should acquire that power?\n\r",ch);
        return;
        }
	if ((victim = ch->fighting) == NULL) {
	...
	act("$n attempts to confuse you.",ch,NULL,victim,TO_VICT, FALSE);
	act("You attempt to confuse $N.",ch,NULL,victim,TO_CHAR, FALSE);
	act("$n attempts to confuse $N.",ch,NULL,victim,TO_NOTVICT, FALSE);
...
void do_earthshatter( CHAR_DATA *ch, char *argument)
{
	...
	act("$n causes the earth to shatter",ch,NULL,NULL,TO_ROOM, FALSE);
