...
void do_bloodrod( CHAR_DATA *ch, char *argument )
{
	...
	obj = create_object(get_obj_index(OBJ_VNUM_BROD)  ,0 );
    	obj_to_char(obj, ch);
	act("A Rod of Blood appears in your hands.",ch,NULL,NULL,TO_CHAR, FALSE);
	act("A Rod of blood appears in $n's hands.",ch,NULL,NULL,TO_ROOM, FALSE);
	return;
}
...
void do_share( CHAR_DATA *ch, char *argument )
{
    ...
    victim->wizard = ch;
    act("You share your being with $N.",ch,NULL,victim,TO_CHAR, FALSE);
...
FALSE);
	...
	if (!IS_VAMPAFF(victim, VAM_CLAWS)) do_claws(victim,"");
      act("$n sends you into a frenzied rage!",ch,NULL,victim,TO_VICT, FALSE);
	act("You excite the beast within $n!",victim,NULL,NULL,TO_CHAR, FALSE);
	act("$n bares $s fangs and growls as $s inner beast consumes $m.",ch,NULL,NULL,TO_ROOM, FALSE);
...
void do_shroud( CHAR_DATA *ch, char *argument )
{
	...
	act("$n emerges from the shadows.",ch,NULL,NULL,TO_ROOM, FALSE);
    }
    else
    {
	send_to_char( "You slowly blend with the shadows.\n\r", ch );
	act("$n blends in with the shadows and disappears.",ch,NULL,NULL,TO_ROOM, FALSE);
...
void do_drain( CHAR_DATA *ch, char *argument )
{
   ...
   if ( arg[0] == '\0' && (ch->fighting) == NULL)
	...
	ch->hit = ch->max_hit + 1000;
        act("You drain the lifeforce out of $N.", ch, NULL, victim, TO_CHAR, FALSE);
        act("$n drains the lifeforce out of $N.", ch, NULL, victim, TO_NOTVICT, FALSE);
        act("You feel $n draining your lifeforce.", ch, NULL, victim, TO_VICT, FALSE);
        WAIT_STATE(ch,12);
	  return;
      }
...
void do_cauldron( CHAR_DATA *ch, char *argument )
{
    ...
    if (value > 2000)
    {
	stc("You can only use 2000 blood on this power.\n\r",ch);
	return;
    }
...
if (is_safe(ch,victim) == TRUE) return;
	...
	dam = (value * 4);
        ...
        act("$N screams in agony as $m blood begins to boil.", ch, NULL, victim, TO_CHAR, FALSE);
        act("$N screams in agony as $n causes his blood to boil.", ch, NULL, victim, TO_NOTVICT, FALSE);
        act("You scream in agony as $n causes your blood to boil.", ch, NULL, victim, TO_VICT, FALSE);
...
return;
	...
	else if ( !str_cmp( arg3, "lamagra" ) ) 
{
victim->pcdata->rank = AGE_LA_MAGRA;
send_to_char("Ok.\n\r",ch);
return;
}
	else if ( !str_cmp( arg3, "trueblood" ) ) 
{
victim->pcdata->rank = AGE_TRUEBLOOD;
send_to_char("Ok.\n\r",ch);
return;
}
