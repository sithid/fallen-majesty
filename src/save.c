...
void save_char_obj( CHAR_DATA *ch )
{
    ...
    {
        sprintf( strback,"%sstore/%s",PLAYER_DIR,  capitalize(ch->pcdata->switchname) );
...
void fread_char( CHAR_DATA *ch, FILE *fp )
{
	...
	case 'A':
            if ( !str_cmp( word, "Act" ) )
            {
               if (!IS_SET(ch->pcdata->version, VERSION_ACT))
               {
		  dummy = fread_number( fp );
		  xCLEAR_BITS(ch->act);
		  ch->act = multimeb( PLR_BLANK, PLR_COMBINE, PLR_PROMPT, -1);
                  SET_BIT(ch->pcdata->version, VERSION_ACT);
               }
               else
                  ch->act = fread_bitvector( fp );
               fMatch = TRUE;
            }
	...
	case 'B':
	    ...
	    KEY( "Bank",        ch->pcdata->bank,               fread_double( fp ) );
	...
	case 'D':
	    KEY( "Damroll",     ch->damroll,            fread_number( fp ) );
           if ( !str_cmp( word, "Deaf" ) )
            {
               if (!IS_SET(ch->pcdata->version, VERSION_DEAF))
               {
                  convert_to_extbv( ch, VERSION_DEAF, fread_number( fp ) );
                  SET_BIT(ch->pcdata->version, VERSION_DEAF);
               }
               else
                  ch->deaf = fread_bitvector( fp );
               fMatch = TRUE;
                break;
            }
	    KEY( "Denied",	ch->denied,		fread_number( fp ) );
	...
	case 'E':
	    ...
	    {
               if (!IS_SET(ch->pcdata->version, VERSION_EXTRA))
	       {
                  convert_to_extbv( ch, VERSION_EXTRA, fread_number( fp ) );
                  SET_BIT(ch->pcdata->version, VERSION_EXTRA);
	       }
   	       else
 	          ch->extra = fread_bitvector( fp );
	       fMatch = TRUE;
	    }
	    break;
	...
	KEY( "Flag4",  ch->flag4, fread_number( fp ) );
	    KEY( "Form",        dummy,               fread_number( fp ) );
	    break;
	...
	if ( !str_cmp( word, "Gifts" ) )
	    ...
	    KEY( "Gold",        ch->gold,               fread_double( fp ) );
	    break;
	...
	case 'L':
	    ...
	    KEY( "Lexp",	ch->lexp,		fread_number( fp ) );
...
//	    KEY( "LastCap",	ch->pcdata->last_cap,	fread_string( fp ) );
	    break;
	...
	case 'Q':
	    ...
	    KEY( "QpBank",       ch->pcdata->bankqp,     fread_double( fp ) );
	...
	KEY( "Rage",    ch->rage, fread_number( fp ) );
	    ...
	    KEY( "RandMight",	dummy,		fread_number( fp ) );
   ...
   KEY( "Smite",       ch->pcdata->smite,      fread_string( fp ) );
	...
	KEY( "SilTol", ch->siltol, fread_number( fp ) );
        if ( !str_cmp( word, "Stackchar" ) )
        {
           ch->pcdata->stackchar = fread_letter( fp );
           fMatch = TRUE;
	   break;
        }
...
*/
    ...
    if ( isalpha(ch->pcdata->title[0]) || isdigit(ch->pcdata->title[0]) || ch->pcdata->title[0] == '#' || ch->pcdata->title[0] == '{' )
		{
...
/*
 * Reads in a double format from a file
 * -Fand
*/
double
fread_double (FILE * fp)
{
  double number;
  bool sign;
  char *ptr;
