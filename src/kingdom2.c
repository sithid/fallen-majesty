...
/*	ROOM_INDEX_DATA *room;
	...
	king_table[i].bleh = fread_number(fp);
	king_table[i].class = fread_number(fp);
	king_table[i].num = i;
...
void do_clanlist(CHAR_DATA *ch, char *argument) {
	...
	sprintf(buf, "#RClan               Leader       PKs     PDs     Ratio    Members#n\n\r" );
	send_to_char(buf,ch);
	sprintf(buf, "#0----               ------       ---     ---     -----	 -------#n\n\r" );
	send_to_char(buf,ch);
	for (i = 1; i <= MAX_KINGDOM; i++) {
	  ...
	  sprintf(buf, "#G%-18s %-12s %-7d %-7d %-4s        %-7d#n\n\r", 
  	  king_table[i].name,
  	  king_table[i].leader_name,
  	  king_table[i].pk,
  	  king_table[i].pd,
...
void do_induct(CHAR_DATA *ch, char *argument) {
	...
	if (king_table[ch->pcdata->kingdom].class != 0)
        if (!IS_SET(victim->class, king_table[ch->pcdata->kingdom].class))
        {
	   stc("That person is of the wrong class to join your clan.\n\r", ch);
           return;
	}

	if (king_table[ch->pcdata->kingdom].altar > 4)
	{
	send_to_char("Your kingdom already has 5 members.\n\r",ch);
	return;
	}
	...
	king_table[ch->pcdata->kingdom].altar += 1;
