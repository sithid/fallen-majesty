...
void do_naturalharmony( CHAR_DATA *ch, char *argument )
{
  if (IS_NPC(ch)) return;
  ...
  {
      stc("You prepare to absorb attacks directed at you!\n\r",ch);
      SET_BIT(ch->newbits, NEW_NATURAL);
      return;
  }
  if (IS_SET(ch->newbits, NEW_NATURAL))
  {
      stc("You will no longer absorb attacks directed at you!\n\r",ch);
...
void do_monkarmor( CHAR_DATA *ch, char *argument )
{
  ...
  if (arg[0] == '\0')
  {
    send_to_char("Please specify which piece of monk armor you wish to make: Ring Collar Robe\n\rHelmet Shorts Boots Gloves Sleeves Cloak Belt Bracer Mask.\n\r",ch);
    return;
  }
  if ( ch->practice <150 )
  {
    send_to_char("It costs 150 points of primal to create monk equipment.\n\r",ch);
    return;
  }
...
void do_deathtouch( CHAR_DATA *ch, char *argument )
{
  ...
  if (ch->move < 100000)
  {
    stc("You do not have the vitality to do this!\n\r",ch);
    return;
  }
  WAIT_STATE(ch, 24);
  act("You place your hands on $N's head and channel negative energy into $m.",ch,NULL,victim,TO_CHAR, FALSE);
  act("$n places $s hands on your head, and you scream in utter pain.",ch,NULL,victim,TO_VICT, FALSE);
  act("$n places $s hands on $N's head and $N screams in pain.",ch,NULL,victim,TO_ROOM, FALSE);
...
void do_healingtouch( CHAR_DATA *ch, char *argument )
{
  ...
  if (ch->move < 100000)
  {
    stc("You need 100,000 move to do this!\n\r",ch);
    return;
  }
  WAIT_STATE(ch, 12);
  ...
  if ( victim == ch )
  {
    ...
    act("$n concentrates, and magical sparks leap out of $s body.",ch,NULL,NULL,TO_ROOM, FALSE);
...
void do_chi( CHAR_DATA *ch, char *argument )
{
  ...
  if ( ch->chi[CURRENT] == 0 )
  {
    ...
    act("$n's body flickers with energy.",ch,NULL,NULL,TO_ROOM, FALSE);
  }
  if ( ch->chi[CURRENT] == 1 )
  {
    stc("Your body pulses with energy.\n\r",ch);
    act("$n's body pulses with energy.",ch,NULL,NULL,TO_ROOM, FALSE);
  }
  if ( ch->chi[CURRENT] == 2 )
  {
    stc("Your body glows with blue energy.\n\r",ch);
    act("$n's body glows with blue energy.",ch,NULL,NULL,TO_ROOM, FALSE);
  }
  if ( ch->chi[CURRENT] == 3 )
  {
    stc("Your body glows bright red.\n\r",ch);
    act("$n's body glows bright red.",ch,NULL,NULL,TO_ROOM, FALSE);
  }
  if ( ch->chi[CURRENT] == 4 )
  {
    stc("Your body flashes with power.\n\r",ch);
    act("$n's body flashes with power.",ch,NULL,NULL,TO_ROOM, FALSE);
  }
  if ( ch->chi[CURRENT] == 5 )
  {
    stc("Your body emits sparks of energy as you fully focus your ch'i.\n\r",ch);
    act("$n's body emits sparks of energy.",ch,NULL,NULL,TO_ROOM, FALSE);
  }
  WAIT_STATE(ch, 12);
  ch->chi[CURRENT]++;
...
void do_chands(CHAR_DATA *ch, char *argument )
{
  ...
  if(IS_ITEMAFF(ch, ITEMA_CHAOSHANDS))
  {
    ...
    act("The swirling colors around your hands disappear.",ch,NULL,NULL,TO_CHAR, FALSE);
    act("The swirling colors around $n's hands disappear.",ch,NULL,NULL,TO_ROOM, FALSE);
    return;
  }
  else
  {
    ...
    act("Swirling colors appear around your hands bringing up shields.",ch,NULL,NULL,TO_CHAR, FALSE);
    act("Swirling colors appear around $n's hands.",ch,NULL,NULL,TO_ROOM, FALSE);
    return;
  }
}
...
void do_guide( CHAR_DATA *ch, char *argument )
{
  ...
  act("You guide $N in the ways of god.", ch, NULL, victim, TO_CHAR, FALSE);
  act("$n guide $N in the ways of god.", ch, NULL, victim, TO_NOTVICT, FALSE);
  act("$n guide you in the way of god.", ch, NULL, victim, TO_VICT, FALSE);
...
FALSE);
  ...
  else
  {
    ...
    act("$n slowly fades into a cloak of holy invisibility..",ch,NULL,NULL,TO_ROOM, FALSE);
...
void do_godsfavor( CHAR_DATA *ch, char *argument )
{
  ...
  if (!IS_SET(ch->newbits, NEW_MONKFAVOR))
  {
    ...
    act("$n is blessed by the Almighty.",ch,NULL,NULL,TO_ROOM, FALSE);
  ...
  if (IS_SET(ch->newbits, NEW_MONKFAVOR))
  {
    ...
    act("$n no longer looks so holy.",ch,NULL,NULL,TO_ROOM, FALSE);
    return;
  }
}
...
void do_godsfire(CHAR_DATA *ch, char *argument)
{
    OBJ_DATA *obj;
    ...
    if (argument[0] == '\0')
    {
	send_to_char("Which item do you wish to invoke the fire of god in?\n\r", ch);
	return;
    }
    ...
    {
        send_to_char("You can only invoke the fire of god in items you own.\n\r",ch);
        return;
    }
