...
void take_item( CHAR_DATA *ch, OBJ_DATA *obj )
{
    ...
    obj_to_char(obj,ch);
    act("You wince in pain and $p falls to the ground.",ch,obj,NULL,TO_CHAR, FALSE);
    act("$n winces in pain and $p falls to the ground.",ch,obj,NULL,TO_ROOM, FALSE);
    return;
}
...
void birth_date( CHAR_DATA *ch, bool is_self )
{
	...
	sprintf(buf,"#n[#0You are #G%d #0years#n][#G%d#0 %s#n][#0and #G%d #0%s old#n]\n\r",
	...
	{
	    if (!IS_EXTRA(ch, EXTRA_LABOUR)) xSET_BIT(ch->extra, EXTRA_LABOUR);
...
void other_age( CHAR_DATA *ch, EXT_BV extra, bool is_preg, char *argument )
{
    char buf[MAX_STRING_LENGTH];
    char buf2[MAX_STRING_LENGTH];
    char mstatus[20];
    ...
    {
	if (yy > 0 || (dd > 0 && mm >= 9) || xIS_SET(extra, EXTRA_LABOUR))
	{
...
*/
    ...
    act("You give $N permission to FUCK on you.",ch,NULL,victim,TO_CHAR, FALSE);
    act("$n has given you permission to FUCK on $m.",ch,NULL,victim,TO_VICT, FALSE);
    return;
}
