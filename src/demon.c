...
void do_inpart( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char(" Shield       [20000] Longsword    [25000] Shortsword  [25000]\n\r",ch);
    ...
    {
 	inpart = DEM_LONGSWORD;
        cost = 25000;
    }

    else if (!str_cmp(arg2,"shortsword"))
    {
 	inpart = DEM_SHORTSWORD;
	cost = 25000;
...
void do_demonarmour( CHAR_DATA *ch, char *argument )
{
    ...
    obj_to_char(obj, ch);
    act("$p appears in your hands in a blast of flames.",ch,obj,NULL,TO_CHAR, FALSE);
    act("$p appears in $n's hands in a blast of flames.",ch,obj,NULL,TO_ROOM, FALSE);
    return;
}
...
*/
    ...
    act("$n sinks into the ground.",ch,NULL,NULL,TO_ROOM, FALSE);
    char_from_room(ch);
    char_to_room(ch,victim->in_room);
    do_look(ch,"");
    send_to_char("You rise up out of the ground.\n\r",ch);
    act("$n rises up out of the ground.",ch,NULL,NULL,TO_ROOM, FALSE);
    return;
}
...
void do_horns( CHAR_DATA *ch, char *argument )
{
	...
	act("$n's horns slide back into $s head.", ch, NULL, NULL, TO_ROOM, FALSE);
	REMOVE_BIT(ch->pcdata->powers[DPOWER_CURRENT], DEM_HORNS);
	return;
    }
    send_to_char("Your horns extend out of your head.\n\r",ch);
    act("A pair of pointed horns extend from $n's head.", ch, NULL, NULL, TO_ROOM, FALSE);
...
void do_tail( CHAR_DATA *ch, char *argument )
{
	...
	act("$n's tail slides back into $s back.", ch, NULL, NULL, TO_ROOM, FALSE);
	REMOVE_BIT(ch->pcdata->powers[DPOWER_CURRENT], DEM_TAIL);
	return;
    }

    send_to_char("Your tail slides out of your back side.\n\r",ch);
    act("A huge tail extends from $n's back.", ch, NULL, NULL, TO_ROOM, FALSE);
...
void do_hooves( CHAR_DATA *ch, char *argument )
{
	...
	act("$n's hooves transform back into $s feet.", ch, NULL,NULL,TO_ROOM, FALSE);
	REMOVE_BIT(ch->pcdata->powers[DPOWER_CURRENT], DEM_HOOVES);
	return;
    }
    send_to_char("Your feet transform into hooves.\n\r",ch);
    act("$n's feet transform into hooves.", ch, NULL, NULL, TO_ROOM, FALSE);
...
void do_wings( CHAR_DATA *ch, char *argument )
{
   ...
   if (!IS_CLASS(ch, CLASS_DEMON) && !IS_SET(ch->special, SPC_CHAMPION))
	...
	if (!str_cmp(arg,"unfold") || !str_cmp(arg,"u"))
	{
	    ...
	    act("$n's wings unfold from behind $s back.", ch, NULL, NULL, TO_ROOM, FALSE);
	...
	else if (!str_cmp(arg,"fold") || !str_cmp(arg,"f"))
	{
	    ...
	    act("$n's wings fold up behind $s back.", ch, NULL, NULL, TO_ROOM, FALSE);
	...
	act("$n's wings slide into $s back.", ch, NULL, NULL, TO_ROOM, FALSE);
	REMOVE_BIT(ch->pcdata->powers[DPOWER_CURRENT], DEM_WINGS);
	return;
    }
    send_to_char("Your wings extend from your back.\n\r",ch);
    act("A pair of wings extend from $n's back.", ch, NULL, NULL, TO_ROOM, FALSE);
...
void do_weaponform( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg[0] == '\0' )
    {
    send_to_char("Change into longsword or shortsword?\n\r", ch);
    return;
    }
    ...
    if (!str_cmp(arg,"longsword"))
    {
    if (!IS_DEMPOWER( ch, DEM_LONGSWORD))
    {
    stc("You need to get the longsword inpart first.\n\r",ch);
    return;
    }
    ...
    else if (IS_DEMPOWER(ch, DEM_LONGSWORD))
    {
    ch->pcdata->powers[DPOWER_OBJ_VNUM] = 33120;
    ...
    {
    if (!IS_DEMPOWER( ch, DEM_SHORTSWORD))
    {
    stc("You need to get the shortsword inpart first.\n\r",ch);
    return;
    }
    if ((obj = create_object(get_obj_index(33121),60)) == NULL)
    {
        send_to_char( "You don't have the ability to change into a weapon.\n\r", ch);
        return;
    }
    else if (IS_DEMPOWER(ch, DEM_SHORTSWORD))
    {
    ch->pcdata->powers[DPOWER_OBJ_VNUM] = 33121;
    ch->pcdata->obj_vnum = 33121;
    do_save(ch,"");
    obj_to_room(obj,ch->in_room);
    act("$n transforms into $p and falls to the ground.",ch,obj,NULL,TO_ROOM, FALSE);
    act("You transform into $p and fall to the ground.",ch,obj,NULL,TO_CHAR, FALSE);
...
void do_eyespy( CHAR_DATA *ch, char *argument )
{
    ...
    {
	act( "You pluck out your left eyeball and throw it to the ground.", ch, NULL, NULL, TO_CHAR, FALSE );
	act( "$n plucks out $s left eyeball and throws it to the ground.", ch, NULL, NULL, TO_ROOM, FALSE );
    }
    else if (!IS_HEAD(ch,LOST_EYE_R))
    {
	act( "You pluck out your right eyeball and throw it to the ground.", ch, NULL, NULL, TO_CHAR , FALSE);
	act( "$n plucks out $s right eyeball and throws it to the ground.", ch, NULL, NULL, TO_ROOM, FALSE );
    }
    else
    {
	act( "You pluck out your left eyeball and throw it to the ground.", ch, NULL, NULL, TO_CHAR, FALSE );
...
void do_cone( CHAR_DATA *ch, char *argument ) {
	...
	level = level * 1.0;
	act("You Blast $N with a cone of fire.",ch,NULL,victim,TO_CHAR, FALSE);
	act("$n Blasts you with a cone of fire.",ch,NULL,victim,TO_VICT, FALSE);
...
obj_to_char(obj, ch);
act("A Stake appears in your hands in a flash of light.",ch,NULL,NULL,TO_CHAR, FALSE);
act("A Stake appears in $n's hands in a flash of light.",ch,NULL,NULL,TO_ROOM, FALSE);
return;
}
