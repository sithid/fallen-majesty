  ...
  if (arg2[0] == '\0')
  {
	...
	if (ch->pcdata->powers[CYBORG_FACE] > 4)
	 send_to_char("Your senses are improved, allowing you to see the stats and location of others.\n\r",ch);
        ...
        send_to_char("Your can see in the night and shadows.n\r",ch);
        else if ((ch->pcdata->powers[CYBORG_FACE] == 1))
        send_to_char("Your can see in the night.\n\r",ch);
      }
    return;
    }
    else if (!str_cmp(arg1,"legs"))
    {
      ...
      send_to_char("You are a true drow warrior, you have supreme fighting skils.\n\r",ch);
...
void do_stuntubes( CHAR_DATA *ch, char *argument )
{
  ...
  one_argument(argument, arg);
    ...
    {
	stc("You need 5000 mana and move to do that.\n\r",ch);
	return;
    }

  if ((victim = get_char_room(ch, arg)) == NULL)
  {
    send_to_char("Venomspit whom?\n\r", ch );
    return;
  }
    WAIT_STATE( ch,     15);
    ch->move -= 5000;
...
void do_cubeform( CHAR_DATA *ch, char *argument )
{
  ...
  if ( IS_SET(ch->newbits, NEW_CUBEFORM))
  {
    ...
    act( "$n turns into a spider droid.",ch,NULL,NULL,TO_ROOM, FALSE);
  ...
  {
    send_to_char("You need 2000 mana and 2000 make yourself an avatar of Lloth.\n\r",ch);
    return;
  }

  else
  {
    act( "$n eyes flicker as an evil presence enter it's body.",ch,NULL,NULL,TO_ROOM, FALSE);
