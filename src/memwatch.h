...
**  along with a description of how to modify the source, and the
version
...
** DISCLAIMER:
**
**  THIS SOFTWARE IS PROVIDED FREE OF CHARGE 'AS IS' AND JOHAN LINDH
MAKES
**  NO EXPRESS OR IMPLIED WARRANTIES WITH RESPECT TO IT. JOHAN LINDH
WILL
...
** 970101 JLI   [2.41 added over/underflow checking after failed
ASSERT/VERIFY]
...
** 970405 JLI   [2.45 fixed bug related with atexit() and some C++
compilers]
** 970723 JLI   [2.46 added MW_ARI_NULLREAD flag]
** 970813 JLI   [2.47 stabilized marker handling]
** 980317 JLI   [2.48 ripped out C++ support; wasn't working good
anyway]
...
** 990217 JLI	[2.55 improved the emergency repairs diagnostics and
NML]
...
**              [mwRelink() could print out garbage pointer (thanks
mac@phobos.ca)]
**				[added array destructor for C++ (thanks
rdasilva@connecttel.com)]
**				[added mutex support (thanks
rdasilva@connecttel.com)]
** 010531 JLI	[2.67 fix: mwMutexXXX() was declared even if
MW_HAVE_MUTEX was not defined]
**
...
**
** To call the standard C malloc / realloc / calloc / free; use
mwMalloc_(),
...
**
**         For use with C++, which allows use of inlining in header
files
**         and class specific new/delete, you must also define 'new' as
**         'mwNew' and 'delete' as 'mwDelete'. Do this *after* you
include
**         C++ header files from libraries, otherwise you can mess up
their
**         class definitions. If you don't define these, the C++
allocations
**         will not have source file and line number information. Also
note,
**         most C++ class libraries implement their own C++ memory
management,
**         and don't allow anyone to override them. MFC belongs to this
crew.
**         In these cases, the only thing to do is to use
MEMWATCH_NOCPP.
**
...
**
** You can disable MEMWATCH's ASSERT/VERIFY and/or TRACE
implementations.
** This can be useful if you're using a debug terminal or smart
debugger.
...
**      is legal in ANSI C, therefore a warning is written to the log
file,
...
**      Detect: Always on CHECK(), but can be detected in several
places.
...
**      structures have been damaged. You are likely using an
uninitialized
**      pointer somewhere in your program, or are zapping memory all
over.
...
**  Mark:
**      The program terminated without umarking all marked pointers.
Marking
**      can be used to track resources other than memory.
mwMark(pointer,text,...)
**      when the resource is allocated, and mwUnmark(pointer) when it's
freed.
**      The 'text' is displayed for still marked pointers when the
program
**      ends.
**      Detect: When MemWatch terminates.
